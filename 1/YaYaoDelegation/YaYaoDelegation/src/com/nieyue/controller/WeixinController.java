...
public class WeixinController {
	...
	public @ResponseBody String connectionWeiXin(@RequestParam("url")String url,HttpSession session){
		String json = WeiXinBusiness.WXJSSDK(ThirdParty.GetValueByKey(ThirdParty.WEIXIN_YAYAO_APPID), ThirdParty.GetValueByKey(ThirdParty.WEIXIN_YAYAO_SECRET), url);
		//雅耀的公众号
	...
	public  void connectionWeiXinLogin(@RequestParam("code")String code,@RequestParam("state")String state,HttpSession session,HttpServletResponse response) throws Exception{
		//如果存在session则返回
		if(session.getAttribute("openid")!=null&&session.getAttribute("user")!=null){
		...
		u.setOpenid(openid);
		//无session 登录不存在存储登录
		if(userService.weixinBaseUserLogin(openid)==null || userService.weixinBaseUserLogin(openid).equals("")){
    		userService.addUser(u);
    	}else{
    	//无session登录 存在直接登录
    	u=userService.weixinBaseUserLogin(openid);
    	}	
